...
long int read_int(void) { // 정수 범위 확인 X
  char buf[32];
...
void add_tv_item_at(int i) {
  ...
  t->description = malloc_wrapper(INIT_DESCR_LEN); // 0x400c14, rbx:t, rax:t->description
...
void modify_description(void) {
  char name[MAX_NAME_LEN];
  char descr[MAX_DESCR_LEN + 8] = { '\0' }; // %rsp+0x20
  item *item;
  size_t new_len;
  void *p;
...
void manage_shopping_mall(void) {
  ...
  while (1) {
    ...
    fgets(buf, sizeof(buf), stdin); // 0x4011f9
    buf[strcspn(buf, "\n")] = '\0';
    if (strcmp(buf, "L") == 0) {
        list_items();
